...
/// Manages the XPC connection to WiredServerHelper, registered via SMAppService.daemon.
///
/// The helper plist (Contents/Library/LaunchDaemons/fr.read-write.WiredServer3.Helper.plist)
/// uses AssociatedBundleIdentifiers for authorization — no SMAuthorizedClients entitlement or
/// provisioning profile required. The helper validates the caller's code signature itself in
/// shouldAcceptNewConnection via setCodeSigningRequirement.
